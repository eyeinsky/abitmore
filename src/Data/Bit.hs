...
bitListIntegral a = go ([], 0) $ zip [0..] $ bitList a
  where
    ...
    go (acc, sum_) ((i, bit_) : rest) =
      if sum_ == a
      then acc
      else case bit_ of
        I -> let sum' = sum_ + (2^i)
             in go (bit_ : acc, sum') rest
        O -> go (bit_ : acc, sum_) rest
    go _ _ = error "TODO"
  ...
  in case overflow of
      ...
      I : rest -> if all (== O) rest
        then case a' of        -- it's a tie
          I : _ -> ceiling_ a'  -- remainder is odd => round up
          O : _ -> (a, False)  -- remainder is even => round down
          []    -> (a, False)  -- same => round down
        else ceiling_ a'        -- round up
  where
    ceiling_ a = let (x, y) = add1 a in (reverse x, y)
...
topBits a n = foldl' (\acc i -> testBit a i : acc) [] $ take n [max_, max_ - 1 .. 0]
  where
    max_ = finiteBitSize a - 1
...
toBitBoolListN a max_ = map (\i -> testBit a i) [max_, max_ - 1 .. 0]
...
class (KnownNat (Width a), Bits a) => FiniteBinary a where
  type Width a :: Natural

bitWidth :: forall a . FiniteBinary a => Int
bitWidth = fromIntegral (natVal @(Width a) Proxy)
